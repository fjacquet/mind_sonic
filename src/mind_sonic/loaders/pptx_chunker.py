...
class PowerPointChunker(BaseChunker):
    ...
    def __init__(self):
        ...
        logger.info("PowerPointChunker initialized successfully with data_type: %s", self.data_type)
    ...
    def get_chunks(self, loader_output, config=None):
        ...
        try:
            ...
            split_docs = self.text_splitter.create_documents([content_text], metadatas=[metadata])
    ...
    def create_chunks(self, loader, src, app_id=None, config=None, **kwargs):
        ...
        Raises:
            Exception: If there's an error during the chunking process
        """
        logger.info("PowerPointChunker.create_chunks called with src: %s, app_id: %s", src, app_id)
        try:
            # Get loader data
            logger.debug("Loader type: %s", type(loader).__name__)
            data_result = loader.load_data(src, **kwargs)
            logger.debug("Loader data result keys: %s", 
            ...
            logger.info("Successfully created %d chunks", len(result.get("documents", [])))
            return result
        except Exception as e:
            logger.error("Error in PowerPointChunker.create_chunks: %s", str(e), exc_info=True)
            raise
