...
from mind_sonic.utils.logging_utils import get_logger, log_function_call, log_crew_execution
...
@CrewBase
class ResearchCrew():
    """ResearchCrew for researching a topic and generating a report.
    
    This crew is responsible for researching a topic and generating a report
    using a researcher agent and a reporting analyst agent.
    """
    ...
    def crew(self) -> Crew:
        ...
        self.logger.info("Creating ResearchCrew with sequential process and memory enabled")
        crew_instance = Crew(
        ...
        self.logger.info(f"ResearchCrew created with {len(self.agents)} agents and {len(self.tasks)} tasks")
